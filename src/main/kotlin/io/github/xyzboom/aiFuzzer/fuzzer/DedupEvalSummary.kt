 ...
 * @param bugPrevented no-dedup程序失败、dedup程序成功 — dedup 成功阻止了已知 bug
 * @param falsePositive no-dedup程序成功、dedup程序失败 — dedup 引入了新问题
 * @param bothFailed no-dedup程序和dedup程序都失败
 * @param bothSuccess no-dedup程序和dedup程序都成功
 */
data class DedupEvalSummary(
    ...
    val falsePositiveSeeds: List<Long> = emptyList(),
    ...
    /** bug 规避率（在触发的种子里，no-dedup失败/dedup成功） */
    val preventRate: Double get() = if (collected > 0) bugPrevented.toDouble() / collected else 0.0
    /** 假阳性率（在触发的种子里，no-dedup成功/dedup失败） */
    val falsePositiveRate: Double get() = if (collected > 0) falsePositive.toDouble() / collected else 0.0
    ...
    fun printReport() {
        ...
        println("False positive:     $falsePositive (${"%.1f".format(falsePositiveRate * 100)}%)")
        ...
        if (falsePositiveSeeds.isNotEmpty()) {
            println("False positive seeds: ${falsePositiveSeeds.joinToString(", ")}")
        }
