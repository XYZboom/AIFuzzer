...
CrossLangFuzzer 的 IR（位于 `tree/gen/` 下的 Kotlin 接口定义）是**由 tree-generator 自动生成**的，不是手写的。生成器位于 `tree/tree-generator/src/main/kotlin/.../TreeBuilder.kt`，使用 Kotlin 编译器源码中的 `generators/tree-generator-common`（shadowJar 打包）从描述配置自动产出接口、实现、Visitor、Builder 文件。

生成的代码文件都带有 `DO NOT MODIFY IT MANUALLY` 头部。修改 IR 只需改 `TreeBuilder.kt` 中的配置后重新生成即可。
...
### 1.3 AiFuzzer IR 的生成方式

AiFuzzer 的 IR 遵循同样的模式：在 `uir-builder/` 模块中编写 Kotlin 配置（类似 `TreeBuilder.kt`），运行后自动生成到 `uir/gen/`。使用者**不手写 IR 接口、实现、Visitor、Builder**，只维护配置文件和序列化等人工编写的部分。

### 1.4 AiFuzzer IR 与 CrossLangFuzzer IR 的差异
