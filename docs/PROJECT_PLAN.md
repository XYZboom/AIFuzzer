...
## 生成器丰富度评估报告 (2026-07-04)

> 详细评估由 subagent 完成，此处纳入项目规划作为持续改进依据。

### 当前状态评分

| 维度 | 评分 | 现状 |
|------|------|------|
| 算子多样性 | 6/10 | 24 个算子，translator 支持 41 个但 17 个未接入 DefaultOps |
| 拓扑结构多样性 | 4/10 | 纯串行链，无分支/fork-join/子图调用 |
| 类型系统覆盖 | 2/10 | IR 类型系统设计完善但生成器几乎未使用（shape/dtype 硬编码）|
| 合法性保证 | 8/10 | DAG + 引用完整性保证，但 fallback 逻辑不严格 |
...
| 优先级 | 改进项 | 预期收益 | 状态 |
|--------|--------|---------|------|
| 🔴 P0 | 补全 DefaultOps（conv, pool, bn, broadcast 等 17 个） | 算子翻倍，覆盖编译器融合场景 | 进行中 |
| 🔴 P0 | 引入分支拓扑（fork-join） | 串行链 → 多分支 DAG | 待开始 |
| 🟡 P1 | 启用 shape/dtype 多样性 | ndim-only → 完整 tensor 类型约束 | 待开始 |
| 🟡 P1 | 丰富 Attribute 生成（随机合法 axis, keepdims 等） | 增加算子参数空间覆盖 | 待开始 |
| 🟢 P2 | 多后端支持（ONNX、IREE） | 跨编译器对比验证 | 待开始 |
| 🟢 P2 | 多图与图间关系（子图调用） | 覆盖复杂编译器场景 | 待开始 |
...
```
1. 修改生成器/翻译器
2. 编译 → 运行 fuzzing（如 50 轮）
3. 收集 BugCollector 报告
4. 分类问题：
   - 生成器 bug → 修复生成器
   - 翻译器 bug → 修复翻译器
   - AI 编译器 bug → 记录到 BugCollector
5. 循环
```
