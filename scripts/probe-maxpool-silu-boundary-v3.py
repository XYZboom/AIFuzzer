...
2. maxpool: test WITH mean downstream (contextual trigger)
"""
import tvm
...
def test_maxpool_mean(shape, k, s):
    """maxpool + mean chain — the actual trigger pattern"""
    bb = relax.BlockBuilder()
    ...
    with bb.function("f", [v]):
        ...
        r = bb.emit(relax.op.mean(p, axis=[-1], keepdims=False))
...
# [2] maxpool k=2 s=2 + mean: test known bug shapes + FP shapes
print("\n[2] MAX_POOL2D(k=2,s=2) + mean")
test_shapes_22 = [
...
for shape in test_shapes_22:
    r = test_maxpool_mean(shape, 2, 2)
    mark = "CRASH" if r == "CRASH" else "OK"
    print(f"  {shape} -> {mark}")

# [3] maxpool k=2 s=1 + mean
print("\n[3] MAX_POOL2D(k=2,s=1) + mean")
test_shapes_21 = [
    (4, 6, 4, 5),   # known bug
...
for shape in test_shapes_21:
    r = test_maxpool_mean(shape, 2, 1)
    mark = "CRASH" if r == "CRASH" else "OK"
    print(f"  {shape} -> {mark}")

# [4] maxpool k=2 s=2 + mean: wider scan
print("\n[4] MAX_POOL2D(k=2,s=2) + mean — wider scan N=1..4, C=1..6, H/W=2..5")
mp22_crashes = []
for N in [1, 2, 3, 4]:
...
# [5] maxpool k=2 s=1 + mean: wider scan
print("\n[5] MAX_POOL2D(k=2,s=1) + mean — wider scan N=1..4, C=1..6, H/W=2..5")
mp21_crashes = []
for N in [1, 2, 3, 4]:
    for C in [1, 2, 3, 4, 5, 6]:
        for H in [2, 3, 4, 5]:
...
print(f"maxpool(k=2,s=2)+mean crashes: {len(mp22_crashes)} shapes")
print(f"maxpool(k=2,s=1)+mean crashes: {len(mp21_crashes)} shapes")
